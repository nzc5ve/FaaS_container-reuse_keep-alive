...
class LambdaScheduler:
    ...
    def find_container(self, d: LambdaData):
        ...
        if (self.eviction_policy == "FTC_S") and (containers_for_the_lambda == []):
    ...
    def calc_priority(self, c, t, update=False):
        ...
        if self.eviction_policy == "FTC_S":
            freq = sum([x.frequency for x in self.container_clones(c)])/(t+1)
    ...
    def AddToPool(self, c: Container, prewarm:bool=False, at_time=None):
        ...
        if mem_size + self.mem_used <= self.mem_capacity:
            ...
            if prewarm and at_time is not None:
                ...
                c.Priority = self.calc_priority(c,at_time)
            else:
                c.last_access_t = self.wall_time
                c.keep_alive_start_t = self.wall_time
                c.Priority = self.calc_priority(c,self.wall_time)
    ...
    def runActivation(self, d: LambdaData, t = 0):
        ...
        if c is None:
            if warm_containers_to_reuse == []: #No warm containers to reuse
                ...
                if self.checkfree(Container(d)) or ((d.run_time - d.warm_time) < waiting_time): #If waiting time is longer
                    ...
                    self.running_c[c] = (t, t+processing_time)
                    self.WritePerfLog(d, waiting_time, d.warm_time, "hit")
        ...
        c.last_access_t = self.wall_time
        new_prio = self.calc_priority(c, t) #, update=True)
        c.Priority = new_prio
